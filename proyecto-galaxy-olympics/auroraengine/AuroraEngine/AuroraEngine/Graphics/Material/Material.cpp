...
static cAssimpTextureMaping kacTextureMapping[] =
{
	...
	{ aiTextureType_EMISSIVE, "Emissive_" },		// The texture is added to the result of the lighting calculation.

													//It isn't influenced by incoming light.

	{ aiTextureType_HEIGHT, "Height_" },			// The texture	is a height map. By convention, higher gray-scale values stand 
													//for higher elevations from the base height.

	{ aiTextureType_NORMALS, "Normals_" },			// The texture is a (tangent space) normal-map. Again, there are several conventions for tangent-space normal maps. 
													//Assimp does (intentionally) not	distinguish here.

	{ aiTextureType_SHININESS, "Shininess_" },		// The texture defines the glossiness of the material. The glossiness is in fact 
													//the	exponent of the specular (phong) lighting equation. Usually there is a 	
													//conversion function defined to map the linear color values in the texture to a suitable exponent.

	{ aiTextureType_OPACITY, "Opacity_" },			// The texture defines per-pixel opacity. Usually 'white' means opaque and 'black' means 'transparency'. 
	...
	{ aiTextureType_LIGHTMAP, "Lightmap_" },		// Lightmap texture (aka Ambient Occlusion). Both 'Lightmaps' and dedicated 'ambient occlusion maps' 
													//are covered by this material property. The texture contains a scaling value for the final color value of a pixel.
	...
	{ aiTextureType_UNKNOWN, "Unknown_" },			// Unknown texture. A texture reference that does not match any of the definitions above is considered to be 'unknown'. 
...
bool cMaterial::Init( const std::string &lacNameID, const std::string &lacFile )
{
	...
	std::string lacEffectName = lhRoot.ToElement()->
	Attribute("effectName");
	std::string lacEffectPath = lhRoot.ToElement()->
	Attribute("effectPath");
	mEffect = cEffectManager::Get().LoadResource(
	lacEffectName, lacEffectPath );
...
void cMaterial::PrepareRender()
{
	...
	// Set the textures
	for ( unsigned luiTextureIndex = 0;
		luiTextureIndex < maTextureData.size();
		++luiTextureIndex )
	{
		lpEffect->SetParam(
